...
#### Install Ignition
Copy the ignition installer into the new disk image (using minimega)
```bash
mm disk inject ignition.qc2 files <ignition_installer>:/root
```
(Note: if the mm command above is not recognized, you need to set up an alias for the docker command: `alias mm="docker exec -it minimega mm"`)
...
When finished, do a proper shutdown on the vm and then run `mm vm flush` to cleanup
...
![Diagram](img/soap-hil.drawio.png)
